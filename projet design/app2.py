       ...
       # cv2.namedWindow("Face Recogniser")
        ...
        while True:
            ...
            if k%256 == 27:
                ...
                # SPACE pressed
                img_name = name1+".png"
                path=r'C:\Users\Han\Desktop\projet design\Training images'
...
def signup():
    if request.method == 'POST':
        ...
        if username in users:
            return "Cet utilisateur existe déjà. Veuillez choisir un autre nom d'utilisateur."
        
        # Enregistrer l'utilisateur dans le dictionnaire
        users[username] = {
            'email': email,
            'password': password
        }
        
        print(f"Utilisateur enregistré : {username}, {email}, {password}")
        return redirect(url_for('loginuser'))  # Redirection vers la page de connexion
...
def get_matches_for_city(ville_id):
    matches = {
        "casablanca": [
            {
                "nom": "Appartement 1", 
                "description": "2 chambres, proche du centre-ville",
                "adresse": "10 Rue de l'Indépendance, Casablanca",
                "images": [
                    "appartement4.jfif"  # Ajout de l'image pour Appartement 1
                ]
            },
            {
                "nom": "Appartement 2", 
                "description": "3 chambres, vue sur la mer",
                "adresse": "20 Avenue Hassan II, Casablanca",
                "images": [
                    "appartement2.webp"  # Ajout de l'image pour Appartement 2
                ]
            }
        ],
        "rabat": [
            { 
                "nom": "Appartement 3",
                "description": "Studio moderne, proche des transports",
                "adresse": "12 Rue des Roses, Rabat",
                "images": [
                    "appartement.jpg"  # Utilisation de 'appartement.jpg' pour Appartement 3
                ]
            },
            {
                "nom": "Appartement 4",
                "description": "Appartement lumineux avec grande terrasse",
                "adresse": "34 Avenue Mohammed VI, Rabat",
                "images": [
                    "appartement1.jpeg"  # Utilisation de 'appartement1.jpg' pour Appartement 4
                ]
            }
        ]
    }
    return matches.get(ville_id, [])




@app.route('/matches/<ville_id>')
def matches(ville_id):
    matches = get_matches_for_city(ville_id)
    return render_template('matches.html', ville_id=ville_id, matches=matches)
